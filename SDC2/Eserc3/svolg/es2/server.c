...
void cleanup() {
    ...
    if(sem_close(named_semaphore) == -1){
        handle_error("Error closing named semaphore");
    }
    if(sem_unlink(named_semaphore) == -1){
...
int main(int argc, char* argv[]) {
    ...
    named_semaphore = sem_open(SEMAPHORE_NAME,O_CREAT | O_EXCL,0666,1);
